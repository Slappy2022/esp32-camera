...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    uint16_t w = resolution_info[framesize].width;
    uint16_t h = resolution_info[framesize].height;
    uint16_t row_s = (resolution_info[FRAMESIZE_UXGA].height - h) / 2;
    uint16_t col_s = (resolution_info[FRAMESIZE_UXGA].width - w) / 2;
    (void)row_s;
    (void)col_s;
    ...
    uint16_t win_w = resolution_info[FRAMESIZE_UXGA].width;
    uint16_t win_h = resolution_info[FRAMESIZE_UXGA].height;
    ...
    for (; i < sizeof(subsample_cfgs) / sizeof(struct subsample_cfg); i++) {
        ...
        if ((win_w * cfg->ratio_numerator / cfg->ratio_denominator >= w) && (win_h * cfg->ratio_numerator / cfg->ratio_denominator >= h)) {
            ...
            row_s = (resolution_info[FRAMESIZE_UXGA].height - win_h) / 2;
            col_s = (resolution_info[FRAMESIZE_UXGA].width - win_w) / 2;
